...
class Dssat(object):
    ...
    def create_xfile(self, ):
        # create base file in site_path
        # copy (maybe hyperlink) base file in main corp file, such as Maize,Wheat.
        pass
    ...
    def create_DSSBatch(self):
        ...
        name_list = []
        variety_numbers = {}
        for fn in os.listdir(os.path.join(self._station_crop_path, str(self.year_idx))):
            if SUFFIXES[self._crop_type.lower()] == os.path.splitext(fn)[-1]:
                name_list.append(fn)
                with open(os.path.join(self._station_crop_path, self.year_idx, fn)) as fp:
                    while True:
    ...
    def run_glue(self, start_epochs, end_epochs, glue_flag=1):
        '''
        IMPORTANT PROMPT:
            PLEASE REFACTOR  INPUT AT LINE 13 OF ~/Tools/GLUE/GLUE.r AS ---->'cultivar.txt'
        :param start_epochs:
        :param end_epochs:
        :param glue_flag:
        :return:
